			...
			<c:choose>
				<c:when test="${content eq 'modify' || content == 'main' }">
					...
					<div class="MypageModify mypage-right">
						...
						<br> <input type="password" class="join-input" name="mem_pw" id="mem_pw" value="${ses.mem_pw}" placeholder="영문(대소문자 구분), 숫자, 특수문자 조합, 9~12자리" onchange="checkPw()" required> <span class="pw_ok"><i class="fa-solid fa-check"></i></span> <span class="pw_rewrite"><i class="fa-solid fa-x"></i></span> <span class="pw_null"><i class="fa-solid fa-x"></i></span> <br>
